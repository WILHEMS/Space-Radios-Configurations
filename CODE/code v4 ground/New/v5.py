    ...
    try:
        ...
        while True:
            ...
            while nrf.data_ready():
                ...
                format_base = 'B' * 60
                if len(payload) == 60 and payload[0] == 0x01:
                    print("running")
                    values = struct.unpack(format_base, payload)
                    #print(f'Protocol: {values[0]}, temperature: {values[1]}, humidity: {values[2]}')
                    ground_list = list(values)
                    base_bytearray = bytearray(ground_list)
                    
                    image_ground = open('nano19.jpg','wb')
                    image_ground.write(base_bytearray)
